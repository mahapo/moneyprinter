...
export class ExchangeBase extends EventEmitter {
  ...
  get orders(): Order[] {
    return this._orders.filter(order => !!order.info.order_link_id).sort((a, b) => b.timestamp - a.timestamp)
  }

  get activeOrders(): Order[] {
    return this.orders.filter(order => !!order.info.order_link_id.includes(this.lastTime))
  }
