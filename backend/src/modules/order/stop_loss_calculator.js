module.exports = class StopLossCalculator {
  ...
  async calculateForOpenPosition(exchange, position, options = { percent: 3 }) {
    ...
    return new Promise((resolve) => {
      if (!position.entry) {
        this.logger.info(
          `Invalid position entry for stop loss:${JSON.stringify(position)}`
        );
        resolve();
      ...
      if (!price) {
        this.logger.info(
          `Empty price for stop loss:${JSON.stringify(position)}`
        );
      ...
      if (!ticker) {
        this.logger.info(
          `Ticker not found for stop loss:${JSON.stringify(position)}`
        );
      ...
      if (position.side === "long") {
        if (price > ticker.ask) {
          this.logger.info(
            `Ticker out of range stop loss (long): ${JSON.stringify(
              position
        ...
        if (price < ticker.bid) {
          this.logger.info(
            `Ticker out of range stop loss (short): ${JSON.stringify(
              position
            )}${JSON.stringify(ticker)}`
          );
